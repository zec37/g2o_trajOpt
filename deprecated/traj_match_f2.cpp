...
public:
    ...
    virtual void computeError()
    {
        ...
        Vector3d estimate = v->estimate();
        Vector2d m = this->measurement();
        double theta = estimate[0];
        double tx = estimate[1];
        double ty = estimate[2];
        double x = ( cos ( theta ) *m[0] - sin ( theta ) *m[1] ) + tx ;
...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i < tr_num; i++){
        ...
        intostr >> traj_index;
        traj_name = "../data_ori/traj" + traj_index + ".txt";
    ...
    /// Load loop data. ///
    string message;
    ifstream loop_file("../data_ori/loops.txt");
...
// ------------------------------------------------------------------- //
	...
	cout << "transform quatenion is " << transform << endl << endl;
        ...
        for(int j = 0; j < traj_size[i]; j++){
            ...
            double x = ( cos(theta) * temp_pose[0] - sin(theta) * temp_pose[1] ) + tx;
            double y = ( sin(theta) * temp_pose[0] + cos(theta) * temp_pose[1] ) + ty;
